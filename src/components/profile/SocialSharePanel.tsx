...
const SocialSharePanel: React.FC<SocialSharePanelProps> = ({
  ...
  // 本地化视图
  const { content } = useLocalizedView('socialShare');
  ...
  const handleShareToSocial = (platform: 'weibo' | 'wechat' | 'qq') => {
    ...
    switch (shareType) {
      case 'profile':
        shareText = `${content?.shareTexts?.profile || '查看我的熊猫习惯个人资料！等级 ' + profile.level}`;
        break;
      case 'achievements':
        shareText = `${content?.shareTexts?.achievements || '我在熊猫习惯中解锁了 ' + achievements.filter(a => a.unlocked).length + ' 个成就！'}`;
        break;
      case 'statistics':
        shareText = `${content?.shareTexts?.statistics || '查看我的熊猫习惯统计数据！已完成 ' + (statistics?.tasks.completed || 0) + ' 个任务'}`;
        break;
    }
    ...
    switch (platform) {
      case 'weibo':
        ...
        break;
      case 'wechat':
        alert(content?.wechatShareTip || '请截图后在微信中分享');
        break;
      case 'qq':
  ...
  const renderProfileShareCard = () => (
    <div className="profile-share-card bg-white p-6 rounded-lg border border-gray-200">
      ...
      <div className="mb-4">
        <div className="text-sm text-gray-500 mb-1">{content?.level || '等级'}</div>
      ...
      <div className="mb-4">
        <div className="text-sm text-gray-500 mb-1">{content?.bio || '个人简介'}</div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">{content?.achievements || '成就'}</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">{content?.tasksCompleted || '已完成任务'}</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">{content?.currentStreak || '连续天数'}</div>
        </div>
      </div>
      
      <div className="mt-4 text-center text-xs text-gray-400">
        {content?.poweredBy || '由熊猫习惯提供支持'}
      </div>
    </div>
  );
  ...
  const renderStatisticsShareCard = () => (
    <div className="statistics-share-card bg-white p-6 rounded-lg border border-gray-200">
      ...
      <h4 className="text-lg font-bold text-jade-600 mb-3">
        {content?.myStatistics || '我的统计数据'}
      </h4>
      
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="stat-item">
          <div className="text-sm text-gray-500 mb-1">{content?.tasksCompleted || '已完成任务'}</div>
          <div className="text-2xl font-bold text-jade-600">{statistics?.tasks.completed || 0}</div>
        </div>
        
        <div className="stat-item">
          <div className="text-sm text-gray-500 mb-1">{content?.currentStreak || '连续天数'}</div>
          <div className="text-2xl font-bold text-jade-600">{statistics?.tasks.streak || 0}</div>
        </div>
        
        <div className="stat-item">
          <div className="text-sm text-gray-500 mb-1">{content?.longestStreak || '最长连续天数'}</div>
          <div className="text-2xl font-bold text-jade-600">{statistics?.tasks.longestStreak || 0}</div>
        </div>
        
        <div className="stat-item">
          <div className="text-sm text-gray-500 mb-1">{content?.achievementsUnlocked || '已解锁成就'}</div>
  ...
  return (
    <div className="social-share-panel bg-white rounded-lg shadow-md p-6">
      ...
      <div className="flex mb-6">
        ...
        >
          {content?.shareTypes?.profile || '个人资料'}
        </button>
        <button
        ...
        >
          {content?.shareTypes?.achievements || '成就'}
        </button>
        <button
        ...
        >
          {content?.shareTypes?.statistics || '统计数据'}
        </button>
      </div>
      ...
      <div className="space-y-4">
        {!shareImage ? (
          ...
          >
            {isGenerating
              ? (content?.generating || '生成图片中...')
              : (content?.generateImage || '生成分享图片')}
          </Button>
        ) : (
            ...
            <div className="flex space-x-2">
              ...
              >
                <span className="flex items-center justify-center">
                  ...
                  {isCopied ? (content?.copied || '已复制') : (content?.copy || '复制')}
                </span>
              </Button>
            </div>
